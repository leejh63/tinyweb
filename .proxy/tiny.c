...
void doit(int fd){
    // 크롬 브라우저에서 url을 통해 mp4파일 경로로 접근시 
    // 브라우저 측에서 연결을 끊는 현상 발생
    // 이 경우 브라우저에서 처음 요청을 document로 보낸후  mp4를 인식하여 
    // 재요청 진행하는데 이때 연결을 한번 끊게 된다.
    // 이 연결이 끊길때 서버가 에러발생하면서 다운된다.
    // 시그널을 무시하는 설정을 진행하면 된다.
